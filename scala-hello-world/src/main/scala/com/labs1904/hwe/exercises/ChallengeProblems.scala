...
object ChallengeProblems {
	/*
		...
		Returns - String
	*/
	def sameString(string: String): String = {
		string
	}
	...
	/*
	 ...
	 Returns - List[String]
*/
	def upper(strings: List[String]): List[String] = {
	...
	Returns - List[Int]
	 */
	def filterNegatives(nums: List[Int]): List[Int] = {
	...
	Returns - List[String]
 */
	def containsCar(strings: List[String]): List[String] = {
		strings.filter((string: String) => string.contains("car"))
	}
	...
	def sumList(nums: List[Int]): Int = {
		nums.reduce((sum: Int, elem: Int) => sum + elem)
	}
 ...
 */
	...
	def minimum(nums: List[Int]): Int = {
		nums.reduce((minVal: Int, currVal: Int) => minVal min currVal)
	}
 ...
 */
	def minimumOption(nums: List[Option[Int]]): Option[Int] = {
		val flattenedNums = nums.flatten
		if (flattenedNums.isEmpty) {
			None
		} else {
			Some(flattenedNums.min)
		}
	}
